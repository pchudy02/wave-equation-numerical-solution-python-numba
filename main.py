...
cellsize = 1
offset = 7  # display size of a cell in pixel
deriv_coffs_space = np.array(savgol_coeffs(offset * 2 + 1, 2, deriv=2, use='dot'), dtype=np.float32)
deriv_coffs_time = np.array(savgol_coeffs(3, 2, deriv=2, use='dot'), dtype=np.float32)
cmap = np.array([np.array(mpl.cm.inferno(i)[:3]) * 256 for i in np.linspace(0, 1, 256)], dtype=np.uint8)


def init_simulation():
    u = np.zeros((3, dimx + offset * 2 + 2, dimy + offset * 2 + 2), dtype=np.float32)
    u[0, dimx // 2, dimy // 2] = 1000
    # The three dimensional simulation grid u[time, dimx, dimy]

    c = 0.1  # The "original" wave propagation speed

    alpha = np.zeros((u.shape[1], u.shape[2]), dtype=np.float32)
...
@cuda.jit(f'void(float32[:,:,:], float32[:,:,:], float32[:,:], float32[:])')
def update(u, result, alpha, deriv_coffs):
    x, y = cuda.grid(2)
    idx = cuda.threadIdx.x
    idy = cuda.threadIdx.y

    deriv_coffs_loc = cuda.shared.array(shape=5, dtype=np.float32)

    # shape=(offset * 2 + 1, offset * 2 + 32, offset * 2 + 32)
    u_loc = cuda.shared.array(shape=(5, 12, 8), dtype=np.float32)
    alpha_loc = cuda.shared.array(shape=(8, 4), dtype=np.float32)
...
@cuda.jit(f'void(float32[:,:,:], float32[:,:,:], float32[:,:], float32[:], float32[:])')
def update2(u, result, alpha, deriv_coffs_space, deriv_coffs_time):
    ...
    if x < u.shape[1] and y < u.shape[2]:
        ...
        if 0 < x < u.shape[1] - 1 and 0 < y < u.shape[2] - 1:
            ...
            u[0, x, y] = (temp / deriv_coffs_time[0]) * 0.8

        elif (x == 0 or x == u.shape[1] - 1) ^ (y == 0 or y == u.shape[2] - 1):
            pass


# @cuda.jit(f'void(float32[:,:,:], float32, float32, uint8[:,:,:], float32[:,:])')
@cuda.jit()
def use_color_map(u, u_max, u_min, pixeldata, cmap):
    x, y = cuda.grid(2)
    ...
    if x < pixeldata.shape[0] and y < pixeldata.shape[1]:
        ...
        i = 0
        while i < 3:
            pixeldata[x, y, i] = cmap[int((u[0, x + 1, y + 1] - u_min) * 256 / u_delta), i]
            i += 1


def place_raindrops(u):
    if (random.random() < 0.02) and True:
...
def main():
    ...
    while True:
        u = np.append(np.zeros((1, u.shape[1], u.shape[2]), dtype=np.float32), u[:-1], axis=0)
        ...
        # place_raindrops(u)
        result = cuda.device_array((u.shape[0], u.shape[1], u.shape[2]), dtype=np.float32)
        ...
        use_color_map[block_grid, thread_block](cuda.to_device(u), u.max(), u.min(), pixeldata, cmap_mem)
